# Sistema de Gestión de Estacionamiento - Parking Lot (Versión Simplificada)
Version en **[🇺🇸 English](README.md)**

## Descripción General

Sistema simple de gestión de estacionamiento que permite entrada, asignación de espacios, salida y cálculo de tarifas para vehículos. El sistema utiliza patrones de diseño OOP para mantener una arquitectura limpia, escalable y fácil de mantener.

**Versión:** Simplificada (sin reservas anticipadas, sin pagos complejos)
**Enfoque:** Educativo - Práctica de Object-Oriented Design (OOD)

---

##  Arquitectura y Patrones de Diseño

### Patrones Implementados

1. **Facade Pattern** (`ParkingLotFacade`)
   - Proporciona una interfaz simplificada para el usuario
   - Orquesta la interacción entre ParkingManager, FareCalculator y Ticket

2. **Manager Pattern** (`ParkingManager`)
   - Gestiona el ciclo de vida de los espacios de estacionamiento
   - Mantiene inventario de espacios disponibles, ocupados, etc.

3. **Strategy Pattern** (`FareCalculator`)
   - Encapsula la lógica de cálculo de tarifas
   - Permite extensión futura con diferentes estrategias de precios

4. **Data Class** (Kotlin)
   - `Vehicle`, `ParkingSpace`, `Ticket` como modelos de datos simples

---

## 📋 Requisitos Funcionales (Simplificados)

### RF-01: Gestión de Tipos de Vehículos ✅

**Descripción:** El sistema soporta múltiples tipos de vehículos.

**Tipos Soportados:**
- `CAR` - Automóvil (tarifa: $5/hora)
- `MOTORCYCLE` - Motocicleta (tarifa: $2/hora)
- `TRUCK` - Camión (tarifa: $10/hora)
- `BUS` - Autobús (tarifa: $1/hora)

**Implementación:**
```kotlin
enum class TypeVehicle {
    CAR, MOTORCYCLE, TRUCK, BUS
}
```

---

### RF-02: Gestión de Espacios de Estacionamiento 

**Descripción:** El sistema administra espacios físicos disponibles.

**Propiedades de un Espacio:**
- `identifier` - ID único (ej: "A1", "B5")
- `parkingLevel` - Nivel/piso del estacionamiento
- `spaceType` - Tipo de espacio (NORMAL, HANDICAPPED, COMPACT, OVERSIZED)
- `status` - Estado actual:
   - `AVAILABLE` - Disponible para estacionar
   - `OCCUPIED` - Ocupado por un vehículo
   - `RESERVED` - Reservado (futuro)
   - `MAINTENANCE` - En mantenimiento
- `availableTypeVehicles` - Lista de tipos de vehículos permitidos

**Criterios de Aceptación:**
- ✅ Se pueden crear espacios de estacionamiento
- ✅ Cada espacio tiene un estado bien definido
- ✅ Se puede consultar disponibilidad en tiempo real
- ✅ Los espacios se asignan automáticamente según tipo de vehículo

---

### RF-03: Entrada de Vehículo ✅

**Descripción:** Un vehículo entra al estacionamiento y se le asigna un espacio.

**Flujo:**
```
Usuario llega con vehículo
        ↓
Sistema busca espacio disponible para ese tipo
        ↓
¿Hay espacio?
├─ SÍ → Asignar espacio + Crear ticket → Retornar ticket
└─ NO → Retornar null (sin espacios disponibles)
```

**Criterios de Aceptación:**
- ✅ Se valida disponibilidad de espacios
- ✅ Se asigna espacio compatible con tipo de vehículo
- ✅ Se genera ticket único con ID
- ✅ Se registra hora de entrada
- ✅ Se cambia estado del espacio a OCCUPIED

**Implementación:**
```kotlin
fun entry(vehicle: Vehicle): Ticket?
```

---

### RF-04: Salida de Vehículo ✅

**Descripción:** Un vehículo sale del estacionamiento y se calcula la tarifa.

**Flujo:**
```
Usuario sale del estacionamiento
        ↓
Sistema obtiene su ticket
        ↓
Calcula duración: exitTime - entryTime
        ↓
Calcula tarifa: FareCalculator.calculateFare()
        ↓
Libera espacio (status = AVAILABLE)
        ↓
Retorna tarifa a cobrar
```

**Criterios de Aceptación:**
- ✅ Se obtiene ticket del vehículo
- ✅ Se registra hora exacta de salida
- ✅ Se calcula duración del estacionamiento
- ✅ Se calcula tarifa correctamente
- ✅ Se libera el espacio

**Implementación:**
```kotlin
fun exit(vehicle: Vehicle): Double?  // Retorna tarifa a cobrar
```

---

### RF-05: Cálculo de Tarifas 

**Descripción:** Sistema de precios basado en tipo de vehículo y duración.

**Modelo de Tarificación:**
```
Tarifa Total = Tarifa Base × Duración (en horas)
```

**Tarifas Base:**
| Tipo de Vehículo | Tarifa por hora |
|------------------|-----------------|
| Automóvil (CAR) | $5.00 |
| Motocicleta (MOTORCYCLE) | $2.00 |
| Camión (TRUCK) | $10.00 |
| Autobús (BUS) | $1.00 |

**Ejemplo de Cálculo:**
```
Automóvil estacionado 3 horas:
  Tarifa = $5.00 × 3 = $15.00

Motocicleta estacionada 2.5 horas:
  Tarifa = $2.00 × 2 = $4.00 (redondeado a horas)
```

**Criterios de Aceptación:**
- ✅ Se calcula tarifa correctamente
- ✅ Considera tipo de vehículo
- ✅ Considera duración del estacionamiento
- ✅ Lanza excepción para tipo de vehículo desconocido

---

##  Clases Principales

### 1. `Vehicle`
Representa un vehículo en el sistema.

```kotlin
data class Vehicle(
    val licensePlate: String,      // Placa del vehículo
    val owner: String,              // Dueño del vehículo
    var typeVehicle: TypeVehicle   // Tipo de vehículo
)
```

---

### 2. `ParkingSpace`
Representa un espacio individual de estacionamiento.

```kotlin
data class ParkingSpace(
    val identifier: String,                      // ID único (ej: "A1")
    val parkingLevel: Int,                       // Nivel/piso
    val spaceType: SpaceType,                    // Tipo de espacio
    var status: StatusParkingSpace,              // Estado actual
    val availableTypeVehicles: List<TypeVehicle> // Tipos permitidos
)
```

---

### 3. `Ticket`
Representa una sesión de estacionamiento.

```kotlin
data class Ticket(
    val ticketId: String,           // ID único del ticket
    val vehicle: Vehicle,            // Vehículo estacionado
    val parkingSpace: ParkingSpace,  // Espacio asignado
    val entryTime: LocalDateTime,    // Hora de entrada
    var exitTime: LocalDateTime?     // Hora de salida (null si activo)
)
```

**Métodos:**
- `isActive(): Boolean` - Indica si el vehículo aún está estacionado
- `getDuration(): Long?` - Calcula duración en horas

---

### 4. `ParkingManager`
Gestor central de espacios de estacionamiento.

```kotlin
class ParkingManager {
    fun addParkingSpace(space: ParkingSpace)
    fun findAvailabeSpaces(typeVehicle: TypeVehicle): ParkingSpace?
    fun getAvailableSpaces(typeVehicle: TypeVehicle): List<ParkingSpace>
    fun getAvailableSpacesCount(typeVehicle: TypeVehicle): Int
    fun hasAvailableSpace(typeVehicle: TypeVehicle): Boolean
    fun releaseSpace(space: ParkingSpace)
    fun getTotalSpaces(): Int
    fun getOccupiedSpaces(): Int
}
```

---

### 5. `FareCalculator`
Calcula tarifas de estacionamiento.

```kotlin
class FareCalculator {
    fun calculateFare(vehicleType: TypeVehicle, durationHours: Long): Double
    fun getBaseFare(vehicleType: TypeVehicle): Double
    fun getAllFares(): Map<TypeVehicle, Double>
}
```

---

### 6. `ParkingLotFacade` 
Facade que simplifica toda la interacción con el sistema.

```kotlin
class ParkingLotFacade(
    private val parkingManager: ParkingManager,
    private val fareCalculator: FareCalculator
) {
    fun entry(vehicle: Vehicle): Ticket?
    fun exit(vehicle: Vehicle): Double?
    fun getActiveTicket(licensePlate: String): Ticket?
    fun getActiveSessionsCount(): Int
}
```

---

##  Caso de Uso Completo

### Escenario: Cliente estaciona auto durante 2 horas

```
1. ENTRADA
   Cliente: "Quiero estacionar mi auto (ABC-123)"
   Sistema:
   ├─ Busca espacio disponible para CAR
   ├─ Encuentra espacio A1
   ├─ Crea ticket #12345
   └─ Retorna: "Ticket generado, espacio A1"

2. [Auto estacionado 2 horas]

3. SALIDA
   Cliente: "Me voy"
   Sistema:
   ├─ Obtiene ticket #12345
   ├─ Registra salida actual
   ├─ Calcula duración: 2 horas
   ├─ Calcula tarifa: $5/hora × 2h = $10
   ├─ Libera espacio A1
   └─ Retorna: "Tarifa: $10.00"
```

---

## 📊 Diagrama UML de Clases

Para visualizar el diagrama completo de clases, revisa el archivo: **[diagrama.puml](./diagrama.puml)**

El diagrama incluye:
- ✅ **3 Enums:** TypeVehicle, SpaceType, StatusParkingSpace
- ✅ **6 Clases principales:** Vehicle, ParkingSpace, Ticket, ParkingManager, FareCalculator, ParkingLotFacade
- ✅ **Relaciones entre clases:** Dependencias, composición, asociaciones
- ✅ **Patrones de diseño anotados:** Facade, Manager, Strategy

**Para visualizar el diagrama PlantUML:**
1. Usa PlantUML en VS Code (extensión PlantUML)
2. O copia el contenido en [plantuml.com](https://www.plantuml.com/plantuml/uml/)
3. O usa `plantuml diagrama.puml` en terminal

---

## 📊 Flujo de Datos Simplificado

```
┌─────────────────────────────────────────────────────────┐
│           ParkingLotFacade (Facade)                    │
├─────────────────────────────────────────────────────────┤
│                                                         │
│  ┌──────────────────┐      ┌──────────────────────┐   │
│  │ ParkingManager   │      │ FareCalculator       │   │
│  ├──────────────────┤      ├──────────────────────┤   │
│  │ • Espacios       │      │ • Tarifas base       │   │
│  │ • Disponibilidad │      │ • Cálculos           │   │
│  └────────┬─────────┘      └──────────────────────┘   │
│           │                                            │
│  ┌────────▼────────────────────────────────────────┐   │
│  │         Ticket (Sesión de estacionamiento)      │   │
│  ├───────────────────────────────────────────────┤   │
│  │ • ID                                           │   │
│  │ • Vehículo                                     │   │
│  │ • Espacio asignado                             │   │
│  │ • Tiempos (entrada/salida)                     │   │
│  └───────────────────────────────────────────────┘   │
│                                                         │
└─────────────────────────────────────────────────────────┘
```

---

##  Estructura del Proyecto

```
src/
├── domain/
│   ├── Vehicle.kt              # Modelo de vehículo
│   ├── ParkingSpace.kt         # Modelo de espacio
│   ├── Ticket.kt               # Modelo de ticket
│   ├── ParkingManager.kt       # Manager de espacios
│   ├── FareCalculator.kt       # Calculador de tarifas
│   └── ParkingLotFacade.kt     # Facade principal
├── enums/
│   ├── TypeVehicle.kt          # Tipos de vehículos
│   ├── SpaceType.kt            # Tipos de espacios
│   └── StatusParkingSpace.kt   # Estados de espacios
└── Main.kt                      # Ejemplo de uso
```

```kotlin
fun main() {
    // Create components
    val parkingManager = ParkingManager()
    val fareCalculator = FareCalculator()
    val parkingLot = ParkingLotFacade(parkingManager, fareCalculator)

    // Add spaces
    val space1 = ParkingSpace(
        identifier = "A1",
        parkingLevel = 1,
        spaceType = SpaceType.NORMAL,
        status = StatusParkingSpace.AVAILABLE,
        availableTypeVehicles = listOf(TypeVehicle.CAR, TypeVehicle.BUS)
    )
    parkingManager.addParkingSpace(space1)

    // Create vehicle
    val car = Vehicle(
        licensePlate = "ABC-123",
        owner = "John Doe",
        typeVehicle = TypeVehicle.CAR
    )

    // ENTRY: Vehicle enters
    val ticket = parkingLot.entry(car)
    if (ticket != null) {
        println("Ticket generated: ${ticket.ticketId}")
        println("Assigned space: ${ticket.parkingSpace.identifier}")
    }

    // [Vehicle is parked...]

    // EXIT: Vehicle leaves
    val fare = parkingLot.exit(car)
    if (fare != null) {
        println("Fare to pay: $$fare")
    }
}
```
David Garcia: (@manuonda)